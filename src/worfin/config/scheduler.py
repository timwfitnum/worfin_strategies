...
class StrategySchedule:
    ...
    run_time_utc: str | None = None  # "13:30" for daily strategies
    interval_minutes: int | None = None  # For intraday: recompute every N minutes
...
def get_intraday_strategies() -> list[StrategySchedule]:
    ...
    return [
        s
        for s in STRATEGY_SCHEDULES.values()
        if s.frequency not in (Frequency.DAILY, Frequency.SWING)
    ]
